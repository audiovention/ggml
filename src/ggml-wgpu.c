...
fn kernel_conv_1d_small_kern_pf16(@builtin(global_invocation_id) global_id: vec3<u32>, 
    ...
    let real_input_len = output_len + d0*(nk - 1u);
...
static const char src_ggml_shader_kernel_conv_1d_small_kern_nk1_pf16[] = MULTILINE(

@compute
@workgroup_size(256)
fn kernel_conv_1d_small_kern_nk1_pf16(@builtin(global_invocation_id) global_id: vec3<u32>, 
    @builtin(workgroup_id) wg_id: vec3<u32>,
    @builtin(local_invocation_id) local_id: vec3<u32>) {
    let apply_tanh = bool(tensor_dimension_params.params[0][3]);
    let has_bias = bool(tensor_dimension_params.params[1][0]);
    ...
    if (has_inject_signal) {
        output += unpack2x16float(bitcast<u32>(get_src3_lin(calc_src_idx(u32(tensor_dimension_params.src[3].ne[0]) - output_len + mult_idx, global_id.y, global_id.z, 3)/2u)));
    }

    let base_src1_offset = input_len - output_len + mult_idx + global_id.z * tensor_dimension_params.src[1].nb[2];

    for (var ic = 0u; ic < input_channels; ic = ic + 1u) {
        let input = unpack2x16float(bitcast<u32>(get_src1_lin((base_src1_offset + ic * tensor_dimension_params.src[1].nb[1])/2u)));
        let kernel = get_src0_pf16(global_id.y, ic, 0u);
        output += input * kernel;
    }
...
static const char src_ggml_shader_kernel_conv_1d_small_kern_div_no_offs_pf16[] = MULTILINE(

@compute
@workgroup_size(256)
fn kernel_conv_1d_small_kern_div_no_offs_pf16(@builtin(global_invocation_id) global_id: vec3<u32>, 
    @builtin(workgroup_id) wg_id: vec3<u32>,
    @builtin(local_invocation_id) local_id: vec3<u32>) {
    let d0 = u32(tensor_dimension_params.params[0][2]);
    let apply_tanh = bool(tensor_dimension_params.params[0][3]);
    ...
    if (has_inject_signal) {
        output += unpack2x16float(bitcast<u32>(get_src3_lin(calc_src_idx(mult_idx, global_id.y, global_id.z, 3)/2u)));
    }
    ...
    for (var ik = 0u; ik < nk; ik = ik + 1u) {
        let in_idx_offset = ik * d0 + base_src1_offset;
        for (var ic = 0u; ic < input_channels; ic = ic + 1u) {
            let input = unpack2x16float(bitcast<u32>(get_src1_lin((in_idx_offset + ic * tensor_dimension_params.src[1].nb[1])/2u)));
            let kernel = get_src0_pf16(global_id.y, ic, ik);
            output += input * kernel;
        }
    }
...
static const char src_ggml_shader_kernel_conv_1d_small_kern_back_filter_large_dil[] = MULTILINE(

var<workgroup> workgroup_data: array<f32, 256>;

@compute
@workgroup_size(256)
fn kernel_conv_1d_small_kern_back_filter_large_dil(@builtin(global_invocation_id) global_id: vec3<u32>,
    @builtin(workgroup_id) wg_id: vec3<u32>,
    @builtin(local_invocation_id) local_id: vec3<u32>) {
    let d0 = u32(tensor_dimension_params.params[0][2]);
    let nk = u32(tensor_dimension_params.dst.ne[2]);
...
const size_t GGML_WGPU_OPERATOR_PARAMS_SIZE_TOTAL = 512;
static_assert(sizeof(struct ggml_wgpu_operator_params) == 512, "ggml_wgpu_operator_params size is wrong");
...
#define GGML_WGPU_DECL_KERNEL(name) \
    ...
    GGML_WGPU_DECL_KERNEL(conv_1d_small_kern_nk1_pf16)
    ...
    GGML_WGPU_DECL_KERNEL(conv_1d_small_kern_back_filter_large_dil)
...
#define GGML_WGPU_ADD_KERNEL(name) \
        ...
        GGML_WGPU_ADD_KERNEL(conv_1d_small_kern_nk1_pf16);
        ...
        GGML_WGPU_ADD_KERNEL(conv_1d_small_kern_back_filter_large_dil);
...
#define GGML_WGPU_DEL_KERNEL(name) \
    ...
    GGML_WGPU_DEL_KERNEL(conv_1d_small_kern_nk1_pf16)
    ...
    GGML_WGPU_DEL_KERNEL(conv_1d_small_kern_back_filter_large_dil)
...
void ggml_wgpu_graph_compute(
    ...
    wgpuQueueWriteBuffer(ctx->queue, ctx->tensor_dimension_operation_params, 0 * GGML_WGPU_OPERATOR_PARAMS_SIZE_TOTAL, 
        &(ctx->tensor_dimension_operation_params_host[0]), gf->n_nodes * sizeof(ctx->tensor_dimension_operation_params_host[0]));
            ...
            case GGML_OP_CONV_1D_SMALL_KERN:
                {
                    ...
                    if (dst->type == GGML_TYPE_F16) {
                        ...
                        if (real_input_len == dst->src[1]->ne[0] && (d0%2==0) && (dst->src[3] == NULL || output_len == dst->src[3]->ne[0])) {
                            GGML_WGPU_ENCODE_KERNEL(conv_1d_small_kern_div_no_offs_pf16, dispatch_x, dispatch_y, dispatch_z)
                        } else {
                            const int32_t base_offset = dst->src[1]->ne[0] - real_input_len;
                            const int32_t inject_offset = dst->src[3] ? (dst->src[3]->ne[0] - output_len): 0;
                            if (1 == nk && 0 == (base_offset % 2) && 0 == (inject_offset % 2)) { // nk=1, we check that base_offset and inject_offset are even but they always are in that case
                                GGML_WGPU_ENCODE_KERNEL(conv_1d_small_kern_nk1_pf16, dispatch_x, dispatch_y, dispatch_z)
                            } else {
                    ...
                    if (dst->type == GGML_TYPE_F16) {
                        ...
                        if ((dst->ne[2] > 1) || (dst->ne[0]*dst->ne[1] > 32)) {
                            if (dst->ne[2] > 1) {
                                ...
                                if (d0 >= 4) {
                                    GGML_WGPU_ENCODE_KERNEL(conv_1d_small_kern_back_filter_large_dil, dst->ne[2], dst->ne[0], dst->ne[1])
                                } else {
                                    GGML_WGPU_ENCODE_KERNEL(conv_1d_small_kern_back_filter, dst->ne[2], dst->ne[0], dst->ne[1])
                                }
                            } else {
...
static void ggml_backend_wgpu_simple_set_tensor(ggml_backend_t backend, struct ggml_tensor * t) {
    struct ggml_wgpu_context * wgpu_ctx = (struct ggml_wgpu_context *)backend->context;
    ggml_wgpu_set_tensor(wgpu_ctx, t);
}

static void ggml_backend_wgpu_simple_get_tensor(ggml_backend_t backend, struct ggml_tensor * t) {
    struct ggml_wgpu_context * wgpu_ctx = (struct ggml_wgpu_context *)backend->context;
    ggml_wgpu_get_tensor(wgpu_ctx, t);
}

static bool ggml_backend_wgpu_simple_add_buffer(ggml_backend_t backend, const char * name, void * data, size_t size, size_t max_size) {
    struct ggml_wgpu_context * wgpu_ctx = (struct ggml_wgpu_context *)backend->context;

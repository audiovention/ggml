...
#define GGML_METAL_DECL_KERNEL(name) \
    ...
    GGML_METAL_DECL_KERNEL(conv_1d_small_kern_simpl_f16);
...
#define GGML_METAL_ADD_KERNEL(name) \
        ...
        GGML_METAL_ADD_KERNEL(conv_1d_small_kern_simpl_f16);
...
#define GGML_METAL_DEL_KERNEL(name) \
    ...
    GGML_METAL_DEL_KERNEL(conv_1d_small_kern_simpl_f16);
    ...
    for (int cb_idx = 0; cb_idx < n_cb; ++cb_idx) {
        ...
        dispatch_async(ctx->d_queue, ^{
            ...
            for (int ind = node_start; ind < node_end; ++ind) {
                ...
                const int64_t  ne30 = src3 ? src3->ne[0] : 0;
                const int64_t  ne31 = src3 ? src3->ne[1] : 0;
                const int64_t  ne32 = src3 ? src3->ne[2] : 0;
                const int64_t  ne33 = src3 ? src3->ne[3] : 0;

                const uint64_t nb30 = src3 ? src3->nb[0] : 0;
                const uint64_t nb31 = src3 ? src3->nb[1] : 0;
                const uint64_t nb32 = src3 ? src3->nb[2] : 0;
                const uint64_t nb33 = src3 ? src3->nb[3] : 0;
                ...
                const enum ggml_type src3t = src3 ? src3->type : GGML_TYPE_COUNT;
            ...
            GGML_ASSERT(false);                                     \
                ...
                switch (dst->op) {
                    ...
                    case GGML_OP_CONV_1D_SMALL_KERN:
                        {
                            ...
                            const int32_t output_channels = dst->ne[1];
                            ...
                            [encoder dispatchThreads:MTLSizeMake(dispatch_x, dispatch_y, dispatch_z) threadsPerThreadgroup:MTLSizeMake(threadgroupSize, 1, 1)];
                        } break;
                    case GGML_OP_SUM:
                        {
                            const int threadgroupSize = 256;
                    ...
                    case GGML_OP_ACC:
                        {
                            ...
                            GGML_METAL_SET_F32_OR_F16_PIPELINE(acc)
                    ...
                    case GGML_OP_CONV_1D_SMALL_KERN_BACK_BIAS:
                        {
                            const int threadgroupSize = 256;
                            GGML_METAL_SET_F32_OR_F16_PIPELINE(conv_1d_small_kern_back_bias)

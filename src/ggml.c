...
inline static void ggml_vec_mul_f32 (const int n, float * z, const float * x, const float * y) 
{
    ...
    for (int i = 0; i < np; i += GGML_F32_STEP) {
        for (int j = 0; j < GGML_F32_ARR; j++) {
            ax[j] = GGML_F32_VEC_LOAD(x + i + j*GGML_F32_EPR);
            ay[j] = GGML_F32_VEC_LOAD(y + i + j*GGML_F32_EPR);
            ay[j] = GGML_F32_VEC_MUL(ax[j], ay[j]);
            GGML_F32_VEC_STORE(z + i + j*GGML_F32_EPR, ay[j]);
        }
    }
...
static void gemm_f32_out_f32(int64_t m, int64_t n, int64_t k,
                             float * A,
                             float * B,
                             float * C,
...
static void ggml_compute_forward_conv_1d_small_kern(
        const struct ggml_compute_params * params,
        const struct ggml_tensor * src0,
        const struct ggml_tensor * src1,
...
void ggml_graph_print(const struct ggml_cgraph * cgraph) {
    ...
    for (int i = 0; i < cgraph->n_nodes; i++) {
        ...
        GGML_PRINT(" - %3d: [ %5" PRId64 ", %5" PRId64 ", %5" PRId64 "] %16s %s (%3d) cpu = %7.3f / %7.3f ms, wall = %7.3f / %7.3f ms / %7.3f GB/s d0: %3d, %3d %3d %3d\n",
                i,
                ...
                bw_gbps, d0, node->src[0]->ne[0], node->src[1]->ne[0], node->src[0]->ne[1]);
    }
...
void ggml_apply_adam_step_to_graph(struct ggml_context *ctx, struct ggml_cgraph* gb)
{
    const int initial_n_nodes = gb->n_nodes;
    int node_add_idx = initial_n_nodes;
    for (int i=0; i < initial_n_nodes; i++) {
        struct ggml_tensor * node = gb->nodes[i];
        if (node->is_param) {
            struct ggml_tensor * new_node = ggml_special_adam_step(ctx, node, node->grad);
            gb->nodes[node_add_idx++] = new_node;
            gb->n_nodes++;
        }
    }
}
...
static inline void ggml_single_adam_step_vec_f32(float* param, const float* g, float* m, float* v, 
    const float beta1, const float beta2, const float beta1h, const float beta2h, 
    const float eps, const float p_decay, const float gnorm, const int n)
{
    int start_index = 0;
#ifdef GGML_SIMD
    const int np = (n & ~(GGML_F32_STEP - 1));

    GGML_F32_VEC aparam[GGML_F32_ARR];
    GGML_F32_VEC ag[GGML_F32_ARR];
    GGML_F32_VEC am[GGML_F32_ARR];
    GGML_F32_VEC av[GGML_F32_ARR];
    GGML_F32_VEC v_gnorm = GGML_F32_VEC_SET1(gnorm);
    GGML_F32_VEC v_beta1 = GGML_F32_VEC_SET1(beta1);
    GGML_F32_VEC v_beta2 = GGML_F32_VEC_SET1(beta2);
    GGML_F32_VEC v_1mbeta1 = GGML_F32_VEC_SET1((1.f-beta1));
    GGML_F32_VEC v_1mbeta2 = GGML_F32_VEC_SET1((1.f-beta2));
    GGML_F32_VEC v_beta1h = GGML_F32_VEC_SET1(beta1h);
    GGML_F32_VEC v_beta2h = GGML_F32_VEC_SET1(beta2h);
    GGML_F32_VEC v_eps = GGML_F32_VEC_SET1(eps);
    GGML_F32_VEC v_1mp_decay = GGML_F32_VEC_SET1((1.f-p_decay));

    for (int i = 0; i < np; i += GGML_F32_STEP) {
        for (int j = 0; j < GGML_F32_ARR; j++) {
            aparam[j] = GGML_F32_VEC_LOAD(param + i + j*GGML_F32_EPR);
            ag[j] = GGML_F32_VEC_MUL(GGML_F32_VEC_LOAD(g + i + j*GGML_F32_EPR), v_gnorm);
            am[j] = GGML_F32_VEC_LOAD(m + i + j*GGML_F32_EPR);
            av[j] = GGML_F32_VEC_LOAD(v + i + j*GGML_F32_EPR);

            am[j] = GGML_F32_VEC_FMA(GGML_F32_VEC_MUL(am[j], v_beta1), ag[j], v_1mbeta1);
            av[j] = GGML_F32_VEC_FMA(GGML_F32_VEC_MUL(av[j], v_beta2), GGML_F32_VEC_MUL(ag[j], ag[j]), v_1mbeta2);

            GGML_F32_VEC_STORE(m + i + j*GGML_F32_EPR, am[j]);
            GGML_F32_VEC_STORE(v + i + j*GGML_F32_EPR, av[j]);
...
static enum ggml_opt_result ggml_opt_lbfgs(
    ...
    ggml_opt_get_params(opt->np, opt->ps, x);
    ...
    {
        ggml_opt_set_params(opt->np, opt->ps, x);

        fx = 0;
        memset(g, 0, sizeof(float)*opt->nx);
        for (int accum_step = 0; accum_step < n_accum; ++accum_step) {
            if (callback) {
                // LBFG-S does not support learning rate -> ignore learning schedule
                float sched = 0;
    ...
    ggml_vec_neg_f32(opt->nx, d, g);

    // ||x||, ||g||
    ggml_vec_norm_f32(opt->nx, &xnorm, x);
    ggml_vec_norm_f32(opt->nx, &gnorm, g);
    ...
    if (opt->just_initialized) {
        ...
        // initial step
        ggml_vec_norm_inv_f32(opt->nx, &opt->lbfgs.step, d);
        opt->lbfgs.j                = 0;
        opt->lbfgs.k                = 1;
        opt->lbfgs.end              = 0;
    ...
    while (true) {
        ...
        ggml_vec_cpy_f32(opt->nx, xp, x);
        ggml_vec_cpy_f32(opt->nx, gp, g);
        ...
        if (ls < 0) {
            ...
            ggml_vec_cpy_f32(opt->nx, x, xp);
            ggml_vec_cpy_f32(opt->nx, g, gp);
        ...
        //   y_{k+1} = g_{k+1} - g_{k}.
        //
        ggml_vec_sub_f32(opt->nx, &lm_s[end[0]*opt->nx], x, xp);
        ggml_vec_sub_f32(opt->nx, &lm_y[end[0]*opt->nx], g, gp);

        // compute scalars ys and yy:
        //     ys = y^t \cdot s    -> 1 / \rho.
        //     yy = y^t \cdot y.
        //
        ggml_vec_dot_f32(opt->nx, &ys, &lm_y[end[0]*opt->nx], &lm_s[end[0]*opt->nx]);
        ggml_vec_dot_f32(opt->nx, &yy, &lm_y[end[0]*opt->nx], &lm_y[end[0]*opt->nx]);
        ...
        for (int i = 0; i < bound; ++i) {
            ...
            ggml_vec_dot_f32(opt->nx, &lm_alpha[j[0]], &lm_s[j[0]*opt->nx], d);
            lm_alpha[j[0]] /= lm_ys[j[0]];
            // q_{i} = q_{i+1} - \alpha_{i} y_{i}
            ggml_vec_mad_f32(opt->nx, d, &lm_y[j[0]*opt->nx], -lm_alpha[j[0]]);
        }

        ggml_vec_scale_f32(opt->nx, d, ys/yy);

        for (int i = 0; i < bound; ++i) {
            // \beta_{j} = \rho_{j} y^t_{j} \cdot \gamma_{i}
            ggml_vec_dot_f32(opt->nx, &beta, &lm_y[j[0]*opt->nx], d);
            beta /= lm_ys[j[0]];
            // \gamma_{i+1} = \gamma_{i} + (\alpha_{j} - \beta_{j}) s_{j}
            ggml_vec_mad_f32(opt->nx, d, &lm_s[j[0]*opt->nx], lm_alpha[j[0]] - beta);
            j[0] = (j[0] + 1)%m;
        }
...
GGML_API void ggml_opt_initialize_opt_params(
    ...
    for (int i = 0; i < gf->n_nodes; ++i) {
        if (gf->nodes[i]->is_param) {
            GGML_PRINT_DEBUG_5("found param %d: grad->op = %d\n", np, gf->nodes[i]->grad->op);

            GGML_ASSERT(np < GGML_MAX_PARAMS);

            opt->ps[np++] = gf->nodes[i];
            nx += ggml_nelements(gf->nodes[i]);
        }
    }

    if ((opt->params.type != params.type) || (opt->nx != nx) || (opt->np != np) || (opt->params.past != params.past) || (opt->params.lbfgs.m != params.lbfgs.m)) {
        int iter = opt->iter;

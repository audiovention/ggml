    ...
    for (int g = 0; g < n_graphs; g++) {
        ...
        for (int i = 0; i < gf->n_nodes; i++) {
            ...
            if (node->op == GGML_OP_SPECIAL_ADAM_STEP) {
                ...
                if (node->src[4]) ggml_allocr_alloc(alloc, node->src[4]);
                if (!ggml_allocr_is_measure(alloc)) {
                    ggml_set_zero(node->src[2]);
                    ggml_set_zero(node->src[3]);
                    if (node->src[4]) { // this initializes the master weights - i.e. the weights that are stored in FP32 for accurate updates, in case the real weights are in FP16
